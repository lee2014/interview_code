...
class Codec:
        ...
        while not_none:
            ...
            for node in stack:
                if node is None:
                    ...
                    val = str(node.val)
                tree_val_array.append(val)
            stack = new_stack

        return "[" + ",".join(tree_val_array) + "]"
    ...
    def deserialize(self, data):
        ...
        if tree_val == '':
            return None
        tree_val_array = tree_val.split(",")
        n = len(tree_val_array)
        tree_root = TreeNode(tree_val_array[0])
        i = 1
        stack = [tree_root]
        while i < n:
            new_stack = []
            for node in stack:
                if node is not None:
                    node.left = TreeNode(tree_val_array[i]) if tree_val_array[i] != 'null' else None
                    new_stack.append(node.left)
                    i += 1
                    if i < n:
                        node.right = TreeNode(tree_val_array[i]) if tree_val_array[i] != 'null' else None
                        new_stack.append(node.right)
                        i += 1
            stack = new_stack
